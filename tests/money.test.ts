...
test("a lump-sum line uses its amount", () => {
  ...
  assert.equal(isMetered(line), false);
...
test("a metered line multiplies quantity by rate", () => {
  ...
  const line = item({ quantity: "65", unit: "ft run", unitPrice: "18.30" });
  assert.equal(isMetered(line), true);
  assert.equal(lineAmount(line), 1189.5);
  assert.equal(unitLabel(line), "65 ft run @ 18.30");
});

test("a half-filled metered line falls back to the lump sum", () => {
  const line = item({ quantity: "2", unit: "nos", amount: "360" });
  assert.equal(isMetered(line), false);
...
test("the metered rate is rounded per line, then summed", () => {
  ...
  const total = documentTotal([
    item({ quantity: "65", unit: "ft run", unitPrice: "18.30" }),
    item({ quantity: "275", unit: "sq ft", unitPrice: "16.50" }),
    item({ quantity: "2", unit: "nos", unitPrice: "180.00" }),
  ]);
